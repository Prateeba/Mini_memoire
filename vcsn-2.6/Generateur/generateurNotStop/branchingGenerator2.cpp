    ...
    while (initial_num_useful_states < num_states){ 
        ...
        for (unsigned i: detail::irange(num_states)){
            ...
            if (temp <= floor(density*1000)) {
                ...
                temp_aut->add_transition(random_state1, states[i], random_label(ls, gen), random_weight()) ;  
                    
                if ( !(is_cycle_ambiguous(temp_aut)) && (is_functional(temp_aut)) && (has_twins_property(temp_aut)) ){ 
                    res->add_transition(random_state1, states[i], random_label(ls, gen), random_weight()) ;  
                        
                }
                else {
                    temp_aut->del_transition(random_state1, states[i], random_label(ls, gen))  ;  
                }    
            }
        }
